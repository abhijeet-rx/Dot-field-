...
public class ExperienceMatcher {
    ...
    public ExperienceResult match(Profile profile, JobRequirements requirements) {
        ...
        double candidateYears = totalDays / 365.25;

        if (candidateYears >= minReqYears) {
            return new ExperienceResult(
                    100,
                    "MEETS_REQUIREMENT",
                    String.format("Candidate has %.1f years of total experience, meeting the required %d years.", candidateYears, minReqYears)
            );
        } else {
            int score = (int) Math.round((candidateYears / minReqYears) * 100.0);
            score = Math.min(100, Math.max(0, score));
            return new ExperienceResult(
                    score,
                    "BELOW_REQUIREMENT",
                    String.format("Candidate has %.1f years of total experience, below the required %d years.", candidateYears, minReqYears)
            );
        }
    }
